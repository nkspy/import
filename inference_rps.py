...
# path=r'E:\ML workspace\RPS\rps_training\rock\rock01-000.png'
...
def prepare(image):
    ...
    return new_array.reshape(-1, IMG_SIZE, IMG_SIZE, 3)
    # prediction = model.predict(np.array(prepare(image_for_inference)))
    #     index = prediction[0].argmax()
    #     print(CATEGORIES[index])
    #     input = cv2.putText(input, '{}'.format(CATEGORIES[index]), (20, 20), font, 1,  (255, 0, 0), 4, cv2.LINE_AA)
...
while (1):
    ...
    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(frame, 'POC', tuple(centerMass), font, 2, (255, 255, 255), 2)
    ...
    image_for_inference = frame[y:y+h,x:x+w]
    prediction = model.predict([prepare(image_for_inference)])
    index = prediction[0].argmax()
    print(CATEGORIES[index])
